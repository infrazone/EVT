...
#
# DESCRIPTION:
#   1. Reads a CSV file listing Subscription IDs and desired tag values.
#   2. For each subscription, checks if tag values are present in CSV.
#   3. Uses `az resource tag --is-incremental` to append missing tags without overwriting existing ones.
#   4. Assumes the CSV has the columns in the order:
